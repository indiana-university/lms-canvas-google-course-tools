# lms-canvas-google-course-tools
...
Insert a record into the `LTI_13_AUTHZ` table with your tool's registration_id (`lms_lti_gct`), along with the client_id 
...
<tr><td>Initiate login URL</td><td>http://localhost:8080/lti/login_initiation/lms_lti_gct</td></tr>
...
| Property                      | Description                                               |
|-------------------------------|-----------------------------------------------------------|
| `gct.impersonationAccount`    | Admin account used by the Google services                 |
| `gct.domain`                  | Domain used to configure the Google Directory Service API |
| `gct.envDisplayPrefix`        | Environment prefix used on groups and folders             |
| `gct.pickerApiKey`            | API key for the Google file picker                        |
| `gct.pickerClientId`          | Client ID for the Google file picker                      |
| `gct.batchNotificationEmail`  | Email address where job notifications will be sent        |
| `gct.googleAuthUrlTemplate`   | Authentication URL wrapper                                |
...
There is a job [RosterSyncJob](src/main/java/edu/iu/uits/lms/gct/job/RosterSyncJob.java) that will perform roster syncing in all configured courses.
